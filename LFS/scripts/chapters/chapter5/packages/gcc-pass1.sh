...
# 日志文件
LOG_FILE="$LFS_LOGS/chapter5_gcc-pass1_$(date +%Y%m%d_%H%M%S).log"
...
extract_package() {
    ...
    # 解压到临时目录
    local build_dir="/tmp/build-gcc-pass1"
    if [ -d "$build_dir" ]; then
        rm -rf "$build_dir"
    fi
    mkdir -p "$build_dir"
...
build_package() {
    ...
    ../configure --prefix=/tools \
            ...
            --disable-libvtv \
            --disable-libstdcxx \
            --enable-languages=c,c++ || error_exit "配置 GCC-pass1 失败"
    
    # 编译
    log "编译 GCC-pass1..."
    make || error_exit "编译 GCC-pass1 失败"
    
    # 安装
    log "安装 GCC-pass1..."
    make install || error_exit "安装 GCC-pass1 失败"
...
main() {
    ...
    # 标记构建完成
    touch "$LFS_STATUS/chapter5_gcc-pass1"
    
    log "GCC-pass1 软件包构建完成！"
}
