...
# 日志文件
LOG_FILE="$LFS_LOGS/chapter5_binutils-pass1_$(date +%Y%m%d_%H%M%S).log"
...
extract_package() {
    ...
    # 解压到临时目录
    local build_dir="/tmp/build-binutils-pass1"
    if [ -d "$build_dir" ]; then
        rm -rf "$build_dir"
    fi
    mkdir -p "$build_dir"
...
build_package() {
    ...
    # 准备编译
    log "准备 Binutils-pass1 编译..."
    ../configure --prefix=/tools \
            --with-sysroot=$LFS_MOUNT \
            --target=$LFS_TGT \
            --disable-nls \
            --enable-gprofng=no \
            --disable-werror || error_exit "配置 Binutils-pass1 失败"
    
    # 编译
    log "编译 Binutils-pass1..."
    make || error_exit "编译 Binutils-pass1 失败"
    
    # 安装
    log "安装 Binutils-pass1..."
    make install || error_exit "安装 Binutils-pass1 失败"
    
    log "Binutils-pass1 构建完成"
}

# 主函数
main() {
    log "开始 Binutils-pass1 软件包构建过程..."
    ...
    # 标记构建完成
    touch "$LFS_STATUS/chapter5_binutils-pass1"
    
    log "Binutils-pass1 软件包构建完成！"
}
