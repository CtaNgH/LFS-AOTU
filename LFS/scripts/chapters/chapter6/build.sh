...
# 日志文件
LOG_FILE="$LFS_LOGS/chapter6_$(date +%Y%m%d_%H%M%S).log"
...
build_package() {
    ...
    # 检查软件包脚本是否存在
    local package_script="$LFS_SCRIPTS/chapters/chapter6/packages/$package.sh"
    if [ ! -f "$package_script" ]; then
        error_exit "软件包脚本不存在: $package_script"
    fi
    
    # 检查是否已经构建完成
    if [ -f "$LFS_STATUS/chapter6_$package" ]; then
